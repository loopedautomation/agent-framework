...
[Voice](voice.md) has the setup, the costs and the fallbacks for both. For a bot invited before you turned voice on, run `af discord-invite` again and open the URL; the invite now carries the permissions to send voice messages, connect to a voice channel and speak in it.
