...
With a top-level `voice` block in the agent file, the trigger handles Discord voice messages: it downloads the audio, transcribes it and runs the transcript through the normal loop, and with `tts` configured the reply comes back as a voice message too. [Voice](voice.md) has the setup and the fallbacks. For a bot invited before voice was enabled, run `af discord-invite` again and open the URL; the invite now carries the permission to send voice messages.
