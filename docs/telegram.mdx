...
Triggers turn an agent into a long-lived service that waits for events, acts on them, delivers the result, and goes idle. The `telegram` trigger connects that loop to Telegram over Bot API long-polling — outbound HTTPS only, so the agent needs no public endpoint. It watches chats and replies to the messages that wake it. (A second transport, `webhook`, trades the always-on poll for an inbound HTTPS endpoint — see [Transports](#transports).)
...
| `transport` | `polling` | How updates arrive: `polling` (long-poll `getUpdates`) or `webhook` (inbound HTTP registered via `setWebhook`). See [Transports](#transports). |
