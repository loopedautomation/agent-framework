...
The email half of the design ships in the repo as two runnable examples: [invoice-bot](https://github.com/loopedautomation/agent-framework/tree/main/examples/invoice-bot) for the Resend forwarding pattern this guide uses, and [helpdesk-bot](https://github.com/loopedautomation/agent-framework/tree/main/examples/helpdesk-bot) for the pulled alternative. This page walks through the rest: the calendar feed, the reminder loop and the spam list.
...
Ready to build it? Start from the [invoice-bot example](https://github.com/loopedautomation/agent-framework/tree/main/examples/invoice-bot) - its README walks through the Resend setup - then swap in the purpose from this page and add the cron triggers, the calendar skill and the `curl` permission from the sections above.
