...
- **`triggers:`** — the events that wake the agent. With triggers, `af run` starts a long-lived service; without, an interactive REPL. → [Discord](discord.mdx) · [Slack](slack.mdx) · [Telegram](telegram.mdx) · [Email](email.mdx) · [Webhook & GitHub](webhook.mdx) · [Cron](cron.mdx)
