...
The Resend transport shines when the agent owns an address outright: point `invoices@example.com` at Resend, route it to the agent, done. It also works for personal mail you'd rather keep at arm's length: a Gmail or Fastmail filter that forwards matching messages ("from my accountant", "subject contains invoice") to the agent's address puts exactly the mail you choose in front of it, and the agent's read-state never touches your own mailbox. The pulled transports remove the forwarding step when you're comfortable letting the agent mark mail read in a folder or label of its own. Both shapes ship as runnable examples: [invoice-bot](https://github.com/loopedautomation/agent-framework/tree/main/examples/invoice-bot) is a Resend address that files what arrives, and [helpdesk-bot](https://github.com/loopedautomation/agent-framework/tree/main/examples/helpdesk-bot) is an IMAP mailbox the agent owns and answers. The [email assistant guide](email-assistant.mdx) composes the trigger with cron and a calendar feed into a personal assistant.
