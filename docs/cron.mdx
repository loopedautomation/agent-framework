...
A cron trigger is the operator's schedule: written in the config, reviewed like code, surviving a wiped data volume. The agent can also create schedules of its own in conversation ("remind me Thursday") when the [`schedules:`](scheduling.md) block is on.
