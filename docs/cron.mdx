...
A cron trigger is the operator's schedule: it lives in the config, gets reviewed like code and survives a wiped data volume. When the [`schedules:`](scheduling.md) block is on, the agent can also create schedules of its own in conversation ("remind me Thursday").
